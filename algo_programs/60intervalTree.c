// Built and print an interval tree
...
// node structure
struct node	{
	int key;	// left interval - not left - dont want to disturb existing implementations
	int height;
...
void fillLevel(struct node *root, int *levelOrder, int *rightOrder, int pos, int max_pos)	{
	...
	if (root == NULL)	{
		if (pos <= max_pos)	{
			...
			fillLevel(NULL, levelOrder, rightOrder, pos*2 + 1, max_pos);
			fillLevel(NULL, levelOrder, rightOrder, pos*2 + 2, max_pos);
		}
		else
	...
	// subtree exist's
		...
		fillLevel(root->lc, levelOrder, rightOrder, pos*2 + 1, max_pos);
		fillLevel(root->rc, levelOrder, rightOrder, pos*2 + 2, max_pos);
	}
}
...
void printIntervalTree(struct node *root)	{
	...
	fillLevel(root, levelOrder, rightOrder, 0, no_nodes - 1);
