// find the element whose rank is given when the element is stored in BBST
...
/*** Find element for a given rank ***/

int findElRank(struct node *root, int rank)	{
	struct node *node = root;
	int temp_rank;

	while (node != NULL)	{
		// rank of the node - rooted at node currently
		if (node->rc != NULL)
			temp_rank = node->rc->num + 1;
		else
			// node is the biggest element node in the tree
			temp_rank = 1;

		if (temp_rank == rank)
			return node->key;
		else if (rank < temp_rank)
			node = node->rc;
		else	{
			rank = rank - temp_rank;
			node = node->lc;
...
int main()	{
	...
	int ele, rank, correct_ele;
	
	while(1)	{
		printf("Enter the rank (-1 to exit): ");
		scanf("%d", &rank);

		if (rank == -1)
			break;

		ele = findElRank(root, rank);
		correct_ele = n - rank;

		printf("Element: %d, Correct Element: %d\n\n", ele, correct_ele);
	}
